...
export default function FinanceModal({ open, title, description, children, onClose }: Props) {
  if (!open) return null

  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/35 p-4">
      <div className="w-full max-w-2xl rounded-3xl border border-theme bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
