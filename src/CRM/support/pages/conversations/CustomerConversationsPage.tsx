    ...
    <div className="grid gap-6 xl:grid-cols-[380px_1fr] fade-up">
      <section className="card overflow-hidden">
        ...
        <div className="divide-y divide-line">
          {conversationThreads.map((thread) => (
            <button key={thread.id} className="w-full px-5 py-4 text-left transition hover:bg-slate-50">
              <div className="flex items-center justify-between gap-2">
                <div className="text-sm font-semibold text-slate-900">{thread.customer}</div>
              ...
              <div className="mt-1 text-xs text-slate-500">{thread.channel} • {thread.subject}</div>
              <div className="mt-2 text-sm text-slate-600">{thread.lastMessage}</div>
            </button>
          ))}
        </div>
      </section>
      ...
      <section className="card p-5">
        <div className="flex items-center justify-between gap-3 border-b border-line pb-4">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">{conversationThreads[0].subject}</h3>
            <p className="mt-1 text-sm text-slate-500">{conversationThreads[0].customer} • {formatSupportDateTime(conversationThreads[0].updatedAt)}</p>
          </div>
          <div className="chip">{conversationThreads[0].channel}</div>
        </div>
        <div className="mt-4 space-y-4">
          <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">{conversationThreads[0].lastMessage}</div>
          <div className="rounded-2xl border border-dashed border-brand-blue/20 bg-blue-50/60 p-4 text-sm text-slate-700">
