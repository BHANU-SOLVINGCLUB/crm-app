    ...
    <div className="space-y-6">
      <section className="card p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-theme-primary">Roles & permissions</h3>
            <p className="mt-1 max-w-2xl text-sm text-theme-secondary">Keep the role model small and explicit so admins can reason about access quickly.</p>
          </div>
          <button className="btn-primary" onClick={() => pushAppToast('Role policy updated.', 'success')}>Save changes</button>
        </div>
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          {roles.map((role) => (
            <div key={role.name} className="rounded-2xl border border-theme bg-theme-surface p-4">
              <div className="text-sm font-semibold text-theme-primary">{role.name}</div>
