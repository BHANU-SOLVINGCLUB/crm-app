...
export default function ProfileSettings() {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h3 className="text-lg font-semibold text-theme-primary">Profile Information</h3>
        <p className="text-sm text-theme-secondary mt-1">Update your personal details and public profile.</p>
      </div>
      ...
      <div className="flex items-center gap-6 pb-6 border-b border-theme">
        <div className="relative">
          ...
          <button className="absolute bottom-0 right-0 h-8 w-8 rounded-full bg-white border border-slate-200 shadow-sm flex items-center justify-center text-theme-secondary hover:text-brand-blue hover:border-brand-blue transition-colors" onClick={triggerUpload}>
            <Camera className="h-4 w-4" />
          </button>
        </div>
        <div>
          <button className="btn-ghost !text-[13px] mb-2" onClick={triggerUpload}>Upload new picture</button>
          <div className="text-[12px] text-theme-secondary">JPG, GIF or PNG. Max size of 800K.</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-5">
        ...
        <div>
          <label className="block text-[13px] font-medium text-theme-primary mb-1.5">Role</label>
          <input type="text" className="input bg-theme-surface text-theme-secondary cursor-not-allowed" defaultValue="Administrator" disabled />
        </div>
      </div>
